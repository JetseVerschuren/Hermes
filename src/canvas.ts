...
export class Canvas {
  ...
  async fetchAnnouncements(courseCodes : string[], startDate : Date | null = null): Promise<Announcement[]> {
    ...
    for (const courseCode of courseCodes) params.append("context_codes[]", courseCode);    
    params.append("active_only", "true"); // Only include published announcements, just in case the token has those permissions
    ...
    if (json.message) {
      console.error(`INVALID RESPONSE FROM API. Request: ${url.toString()}, response: `, response);
